...
new = malloc(sizeof(struct ArtigoInFile));
	
	new->id = bt2/sizeof(struct ArtigoInFile); //new->id fica com o nº de bytes lidos até ao final do ficheiro strings
...
art = structArtigoFile(&fileARTGS,&fileSTRS,nome,preco);
	...
	/*+++++++++++++++++++++++++++++++++++++++
	char buff[120];
	lseek(fileARTGS,art->id,SEEK_CUR);
	readln(fileARTGS,buff,50);

	printf("COdigo do artigo:%s",buff);
	++++++++++++++++++++++++++++++++++++*+++
*/
