...
int fdStr = open("STRINGS.txt", O_RDONLY, 0777);
	...
	if(fdStr>-1){
		int nbEnd = lseek(fdStr,0,SEEK_END);
		int nbLocal = lseek(fdStr,edr_nome,SEEK_SET);
		...
		if(nbLocal<nbEnd){
			...
			write(1,"O endereço que aponta para a posição do nome na ficheiro STRINGS.txt não existe!\n",83);
			exit(0);//----------------------------------------------------------------------------------------HA O RISCO DE HAVER PROBLEMAS POR CAUSA DESTE EXIT()
		}
	}else{
		catchError(ERROR_4);
	}
