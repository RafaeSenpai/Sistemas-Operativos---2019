...
int runAggregation(){//----------------------------------------------------------------------------------------OPERACIONAL
	int fdLog = open(LogFileLocal, O_RDWR,0777);
	...
	if(fdLog==-1){//<--- se o ficheiro log, que indica a ultima venda agregada não existir
		int fdLog = open(LogFileLocal,O_CREAT | O_RDWR,0777);//<<--- cria o ficheiro log e guarda dentro dele o valor da variavel nLastSaleAgg que tem o valor 0, que por sua vez identifica que a agregação teŕa que começar a ser feita logo desde do inicio do ficheiro
		...
		printf("NÃO ENCONTROU FICHEIRO - Valor existente no ficheiro log!:%d\n", nLastSaleAgg);
		/*O gajo se nao existir nenhum ficheiro log ele cria e guarda a posição zero, mas nao faz nenhuma agregação(apesar de gerar o ficheiro de agregação), se correr novamente sem eliminar a pasta de agregação e o ficheiro log, o gajo já cria o fichero de agregação corretamente*/
	}else{
		lseek(fdLog,0,SEEK_SET);
		read(fdLog,&nLastSaleAgg,sizeof(int));//<--- caso o ficheiro log já exista, entao apenas lê o seu conteudo e obtem a ultima posição agregada existente no ficheiro
		printf("ENCONTROU FICHEIRO - Valor existente no ficheiro log!:%d\n", nLastSaleAgg);
	}

	

	if(fdLog>-1){
		printf("Entrou no fdLog\n");
		...
		if(fdSales>-1){
			...
			if(sale){
				if(fdAgg>-1){
					printf("Dentro do agregador a posição no ficheiro vendas contida no ficheiro log é:%d\n",nLastSaleAgg);
